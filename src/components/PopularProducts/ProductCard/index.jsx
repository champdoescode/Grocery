...
import { useCart } from '../../../Contexts/CartContext'; // Adjust the path as per your actual folder structure
...
const Modal = ({ product, onClose }) => {
  ...
  const handleAddToCart = () => {
    ...
    // Call addToCart with product, selected size, quantity, and total price
    addToCart(product, selectedSize, quantity, totalPrice);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg max-w-2xl w-full relative">
        {successMessage && (
          <div className="absolute top-4 right-4 bg-green-500 text-white py-2 px-4 rounded">
            {successMessage}
          </div>
        )}
        <button onClick={onClose} className="absolute top-4 right-4 bg-white border text-black hover:text-gray-700">
          &times;
        </button>
        <div className="flex">
          <div className="w-1/2">
          ...
          <div className="w-1/2 pl-6">
            ...
            <div className="flex items-center mb-4">
              {Array.from({ length: 5 }, (_, i) => (
                <svg
                  key={i}
                  className={`w-5 h-5 fill-current ${i < Math.floor(product.rating) ? 'text-yellow-500' : 'text-gray-300'}`}
                ...
                {['500gm', '1kg', '2kg', '5kg'].map((size) => (
                  <button
                    ...
                    className={`px-3 py-1 border rounded ${
                      size === selectedSize ? 'bg-[#64b496] text-white' : 'bg-white text-black border-gray-300'
                    }`}
                  >
                    {size}
                  </button>
            ...
            <div className="flex items-center">
              <input
                ...
                min={1}
                className="w-16 px-2 py-1 border rounded mr-4 bg-white text-black"
              />
              <button className="bg-[#64b496] text-white px-4 py-2 rounded" onClick={handleAddToCart}>
                Add To Cart
              </button>
            </div>
          </div>
...
const ProductCard = () => {
  ...
  const { addToCart } = useCart() // Access addToCart function from useCart context
  const [successMessage, setSuccessMessage] = useState('');
  ...
  const handleAddToCart = (product) => {
    addToCart(product, '500gm', 1); // Default size and quantity for initial implementation
  ...
  return (
    <>
      ...
      <div className="flex w-full px-16">
              ...
              <div key={index} className="relative bg-white border border-gray-300 rounded overflow-hidden shadow-lg text-black text-sm">
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col p-2 bg-black bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity">
                    ...
                    <FaShoppingCart className="text-white text-2xl cursor-pointer self-end mb-" onClick={() => handleAddToCart(product)} />
                  </div>
                </div>
        ...
        {showModal && <Modal product={selectedProduct} onClose={() => setShowModal(false)} onAddToCart={handleAddToCart} />}
      </div>
      {/* Display success message at top right corner */}
      {successMessage && (
         <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-green-500 text-white py-3 px-6 rounded-lg shadow-lg flex items-center">
         <FaCheckCircle className="text-2xl text-green-700 mr-2" />
